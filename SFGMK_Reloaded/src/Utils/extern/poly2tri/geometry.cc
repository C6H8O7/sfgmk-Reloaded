...
{
     double runtime=(double)1000l*(tv1.time - tv0.time)+(tv1.millitm - tv0.millitm ) / 1000l;
...
BDMFile::BDMFile(const ClipperLib::Path& _Polygon, const ClipperLib::Paths& _Holes)
{
	const double infinity = 1.0e38;
	_xmin = _ymin = infinity;
		_xmax = _ymax = -infinity;
	
	//Extrait les points des polygones fournis par clipper
	ComputePolys(_Polygon, _Holes);
...
Polygon::Polygon(const ClipperLib::Path& _Polygon, const ClipperLib::Paths& _Holes) : BDMFile(_Polygon, _Holes)
{
	initializate();
	_debug = false;
}
