...
namespace WorkspaceView
{
	...
	ThijzWorkspaceCable::ThijzWorkspaceCable(Node* Input, Node* Output, int FromIndex, int ToIndex)
	: Cable(Input, Output, FromIndex, ToIndex)
	{
		ConnectCable(Input, Output, FromIndex, ToIndex);
	}

	void ThijzWorkspaceCable::Draw(wxPaintDC* dc, const State& State)
	{
	...
	: Node(wxRect(Position.x-Radius, Position.y-Radius, Radius*2.0f, Radius*2.0f), InputCount, OutputCount, Title), Radius(Radius)
	{
	...
	void ThijzWorkspaceNode::Draw(wxPaintDC* dc, const State& State)
	{
	...
	Node* ThijzWorkspaceFactory::CreateNode(const wxPoint& Position, float Radius, int InputCount, int OutputCount, const wxString& Title)
	{
		return new ThijzWorkspaceNode(Position, Radius, InputCount, OutputCount, Title);
	}

	Node* ThijzWorkspaceFactory::CreateNode(const wxRect& Area, int InputCount, int OutputCount, const wxString& Title)
	{
	...
	Cable* ThijzWorkspaceFactory::CreateCable(Node* Input, Node* Output, int FromIndex, int ToIndex)
	{
